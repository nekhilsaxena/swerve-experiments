...
void SwerveDriveUtil::submitRequest(const DriveRequest &request, bool queueIfBlocked)
{
    ...
    // Direct control bypasses the request system
    ...
    if (canPreempt(request))
    {
        ...
        {
            if (m_onRequestStarted)
                m_onRequestStarted(request);
...
void SwerveDriveUtil::sendToDriveTrain(double dt)
{
    ...
    else if (m_currentRequest.mode == DriveMode::TARGET_HEADING && m_currentRequest.targetHeading.has_value())
    {
    ...
    else if (m_currentRequest.mode == DriveMode::TARGET_POSE && m_currentRequest.targetPosition.has_value() && m_currentRequest.targetHeading.has_value())
    {
