...
void DriveToPoseCommand::initialize()
{
    ...
    if (m_initialDistance < 0.001)
        m_initialDistance = 0.001;
...
void RotateToHeadingCommand::initialize()
{
    ...
    if (m_initialAngleError < 0.001)
        m_initialAngleError = 0.001;
...
void FollowPathCommand::execute(double dt)
{
    if (m_currentIdx < m_waypoints.size())
    {
        ...
        double velocityErr = (m_robot->getState().velocity - cur.velocity).magnitude();

        if (dist < cur.toleranceM && headErr < cur.toleranceRad && velocityErr < cur.toleranceVel)
        {
            m_currentIdx++;
            if (m_currentIdx < m_waypoints.size())
            {
...
std::string SequentialCommandGroup::getStatus() const
{
    if (m_currentIdx < m_commands.size())
    {
        return "Step " + std::to_string(m_currentIdx + 1) + "/" + std::to_string(m_commands.size()) + ": " + m_commands[m_currentIdx]->getStatus();
    }
